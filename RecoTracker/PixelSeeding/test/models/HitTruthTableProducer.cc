...
void HitTruthTableProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  // Pixel merged-hit labeling, which the OT loop above leaves at tpKey = -1. A legacy pixel rechit
  // built from cluster clust in det gind = geom.idToDetUnit(detid)->index() occupies pixel-SoA index
  // moduleStart[gind] + clust.originalId(), and the merger copies pixel hits and their moduleStart
  // verbatim, so that index is also the merged-hit index. Only indices < offsetStubs are written.
