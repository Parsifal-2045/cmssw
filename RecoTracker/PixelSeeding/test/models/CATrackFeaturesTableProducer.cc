...
private:
  void produce(edm::StreamID, edm::Event& iEvent, const edm::EventSetup& iSetup) const override {
    ...
    for (int it = 0; it < nTracks; ++it) {
      ...
      // The deployed extras come from fill()'s extras, not from the host walk below, which skips
      ...
      const bool ok = caTrackFeatures::fill(
          hitsBegin, hitsEnd, hh, nHitsTot, float(tracks[it].nLayers()), tracks[it].chi2(), feat, &extras, otViewPtr);
      if (!ok)
        continue;
      ...
      ex[0][it] = extras.meanStubKappa;
      ex[1][it] = extras.leverArm;
      ex[2][it] = extras.rMax;
      ex[3][it] = extras.rzChi2;  // -1 when undefined
    ...
    for (int f = 0; f < 7; ++f)
      table->addColumn<float>(kExtraNames[f],
                              ...
                              f < 4 ? "deployed CA track feature (PixelTrackFeaturesSoA columns 28-31, from "
                                      "caTrackFeatures::fill's extras -- OT-aware, bit-identical to the kernel)"
