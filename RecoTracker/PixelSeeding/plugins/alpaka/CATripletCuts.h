...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  struct TripletCuts {
    ...
    // Tangent of the three-point circle at one stub, as a half-curvature. The circle through the
    // triplet's three hits knows the track's transverse impact parameter, so its tangent at the
    // stub's own radius already contains the d0/r^2 term the stub measures, so the comparison below
    // is free of d0. Comparing the stub curvature with the 1-3 chord instead would put
    // kappa/2 + d0/r^2 against kappa/2 + d0/(r1 r3), a difference that grows with the displacement.
    // cosdir(x,y) is the unit radial vector of the circle at (x,y) up to the sign of the curvature;
    // its perpendicular, oriented outwards, is the tangent, and sin(beta)/r with beta the angle to
    // the radial direction is the same quantity as the stub's own d/sqrt(1 + r^2 d^2).
    ...
    ALPAKA_FN_ACC ALPAKA_FN_INLINE static bool accept(
      ...
      if constexpr (std::is_same_v<pixelTopology::Phase2OTStubs, TrackerTraits>) {
        ...
        // Inline per-triplet DNN gate (compile-time weights in CATripletDNNWeights.h, evaluated by
        // CATripletDNN.h): rejects cut-accepted triplets whose score is below threshold, and is a
        // no-op with useTripletDNN off. It gates pixel-only triplets too, through the sentinel
        // features above; one model per iteration covers both regimes (nStubs is a feature). The
        // feature vector is 18 raw quantities + 11 derived ones, whose order and formulas (the
        // 1e-12 eps conventions included) must match test/models/train_triplet_dnn.py.
        // In a CA_TRIPLET_DUMP build the reject is compiled out for the dumped iteration, so every
        // cut-accepted triplet is dumped whatever model is compiled in.
