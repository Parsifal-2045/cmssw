          ...
          for (uint32_t pBase = 0; pBase < nPairs; pBase += warpSize) {
            ...
            if (predicate) {
              ...
              if (out < maxStubIdx) {
                ...
                // Zero-bend reference of the pair: a straight track from the beam spot has the same
                // azimuth on both sensors, so the other hit is compared with the point where the ray
                // through the position hit crosses the other sensor's plane. The other hit measures only
                // its local x; its local y is the centre of its strip segment (on a PS stack the pixel
                // measures it instead), so it is moved along its own local y to the crossing point's
                // along-strip coordinate. Wherever local y has a radial component (tilted barrel, every
                // disc) an along-strip offset dy enters the azimuth difference as x * dy / r^2: for a strip
                // centre that is larger than the bend itself, and the ray's own travel along the strip
                // across the stack, sep * r / z on a disc, is of the order of the 2 GeV bend.
                // The pair's along-strip error becomes the position hit's (already equal on 2S).
                ...
                if (!bendValid) {
                  ...
                  // Precision-only azimuthal bend error: the three coordinates the pair actually
                  // measures, propagated into dPhiDr = dphi / dr_effective.
                  //
                  // The two local x readings enter through the azimuths alone. The along-strip
                  // coordinate is measured once -- by the macro-pixel on a PS stack, nowhere on a 2S
                  // stack, where it is the centre of the 5 cm strip segment -- because the other hit is
                  // taken on the beam-spot ray through the position hit, so its own strip centre has
                  // dropped out. Where the strip has a radial component (every disc, the tilted barrel)
                  // that one unmeasured coordinate moves the bend twice: it moves both azimuths, and it
                  // moves the radial lever dr_effective = separation / (cosTilt + sinTilt z/r) that the
                  // azimuth difference is divided by. On a disc both are -dPhiDr/r, so the bend carries
                  // a relative error of 2 sigma_r / r -- for a 2S disc stub, 3 to 5 % of the bend, the
                  // largest term the low-pT stubs have.
                  {
