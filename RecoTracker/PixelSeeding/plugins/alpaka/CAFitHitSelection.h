...
// Compile-time selection of which hits the BrokenLine fit uses and of how tracks are binned by
// multiplicity. Track finding is unaffected: it stays IT+OT; only the fit and the multiplicity
// binning that feeds it are restricted to the selected hit subset.
//
...
namespace caFitHitSel {
  ...
  // terminator. Such an id indexes the raw OT source, not this merged hit SoA, so it is neither part of
  // the count nor a terminator. Set only by the extension's verify guard.
