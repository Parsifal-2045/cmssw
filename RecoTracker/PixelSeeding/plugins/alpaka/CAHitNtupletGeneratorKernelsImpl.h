    ...
    ALPAKA_FN_ACC void operator()(Acc1D const &acc,
      ...
      for (auto it : cms::alpakatools::uniform_elements(acc, foundNtuplets->nOnes())) {
        ...
        if constexpr (std::is_same_v<pixelTopology::Phase2OTStubs, TrackerTraits>) {
          ...
          // inputs of the network (caFitChi2, logChi2Stub). Feature order is documented in CATrackDNNWeights.h.
          ...
          if (useTrackDNN) {
            ...
            if (featOk) {
              ...
              const float dnnScore = (trackBank == DnnBank::kPrompt)
                                         ? caTrackDNN_eval::score<DnnBank::kPrompt>(featArray.data())
                                         : caTrackDNN_eval::score<DnnBank::kDisplaced>(featArray.data());
