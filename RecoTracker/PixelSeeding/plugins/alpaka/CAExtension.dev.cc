...
namespace ALPAKA_ACCELERATOR_NAMESPACE::caExtension {
  ...
  // How far the crossing may sit from a hit along the secondary row and still be the same particle.
  // The sensor's coordinate along that row is not a Gaussian measurement: a strip (and a pixel along
  // its length) reports the centre of a segment the track crossed anywhere inside, so the reading is
  // uniform over that segment. A uniform variable of variance V has support exactly +-sqrt(3 V), so
  // the hit can only belong to a crossing within sqrt(3 Rss) of it, plus the road's own spread at the
  // same eps. On a PS module this is a fraction of a millimetre and the chi2 already binds; on a 2S
  // module, where the strip is centimetres long, the Gaussian tail of the same variance reaches 2.9x
  // further than the strip itself and admits hits the sensor cannot have produced.
  template <typename TAcc>
  ...
  // Does the secondary reading still separate candidates, or is it only a window? The sensor's
  // coordinate along the strip (z in the barrel, r on a disc) is not a Gaussian measurement: the
  // reading is the centre of a segment the track crossed anywhere inside, uniform over it, with
  // support exactly +-sqrt(3 Rss). Inside that support the likelihood is flat, so the row separates
  // nothing once its support is wider than the road it is compared against -- and a chi2 that keeps
  // the row then hands the candidate a degree of freedom whose residual is free, which is how a 2S
  // strip (centimetres long) outranks an honest candidate. The comparison is made at the same eps
  // as everything else: the row stays a chi2 row while 3 Rss is below the road's own spread
  // qGate1 * M_ss, and becomes the window extSecSupport otherwise. On a PS module or a pixel the
  // support is a fraction of a millimetre and this is never true.
  ...
  // Per-candidate layer walk and windowed hit search. Candidates come from candList (slot j -> tuple
  // id), hit lists are read through hitOffsets/TrackHitSoA, and each accepted extra records its gate
  // chi2 for the arbitration pass. One block per candidate, kExtFindLanes lanes cooperating; the two
  // hot loops are split across the lanes with shared-memory argmin reductions:
  //   - candidate ordering: each lane computes, for every uncovered layer, a cheap geometric
  //     nearest-approach proxy distance to the layer's (r,z) envelope; the walk picks at each step
  //     the min-(proxy dist, then L) unvisited confirmed-reachable layer. Unreachable layers do not
  //     count toward the K = maxWalkLayers budget. The proxy approximates the crossing distance, so
  //     in corner cases the visit order can differ from a full-predict ordering, the physics not;
  ...
  //     best. Both reductions are argmin only, over a strict total order, so the accepted hit does
  //     not depend on the lane partition and no floating-point summation is introduced.
  // Lane 0 owns the sequential state, the other lanes only run the two parallel maps. On the serial
  // backend the lanes are elements of a single thread, so serial validation exercises the same logic.
  struct Kernel_extFindExtras {
    ...
    ALPAKA_FN_ACC void operator()(
      ...
      for (uint32_t j : cms::alpakatools::uniform_groups(acc, nC * nLanes)) {
        ...
        // Walk the candidate layers in (proxy dist, then L) order: the full predict is confirmed in
        // one lane-parallel batch the first time the confirmed set is dry, every candidate layer at
        // once with each lane owning L = lane, lane+nLanes, ... Since a visit is chosen by the
        // select-min over confirmed layers, confirming them all at once changes neither the visit
        // order nor the physics. Layers whose predict fails the envelope/arc test are consumed
        // without spending the K = maxWalkLayers budget, and after the eager round each step is a
        // lane-parallel select plus one window scan. shReachable states: 0 = covered or confirmed
        // unreachable, 1 = candidate, 2 = confirmed reachable. shWalkDone: 0 = scan shCurrentL,
        // 1 = walk finished, 2 = eager-confirm pending.
        while (true) {
          ...
          // Per-(candidate,layer) linearization of the crossing quantities. The per-hit scan below
          // crosses the layer at every in-window hit's own radius rh (barrel) or z (endcap), where
          // evaluating predictOn{Barrel,Endcap} exactly would cost about a dozen transcendentals per
          // hit over 40-80 hits per window. The crossing quantities (phi, secondary, arcS) are
          // instead expanded about the layer reference R0/Z0 to a Taylor polynomial (phi and
          // secondary 2nd order, arcS 1st) whose coefficients are the analytic derivatives of the
          // same closed form, so the per-hit work collapses to a few FMAs. A per-layer fallback
          // (shUseLin=0) keeps the exact predict for near-tangential or Taylor-stressed layers.
          //
          ...
          for (auto element : cms::alpakatools::uniform_group_elements(acc, j, nC * nLanes)) {
            ...
            // The bending field to cross THIS layer in, from the same (Bz,Br) map the fits use,
            // instead of one scalar per host: over a 60 cm forward gap the field falls by a few percent
            // and the road centre moves by O(cm). The walk rebuilds the WHOLE helix from the PCA in one
            // constant field, so the constant that reproduces the crossing POSITION is the average of
            // B_bend over the path from the PCA weighted by (s_end - s): the transverse offset is the
            // double integral of the curvature, x(s_end) = int_0^s_end kappa(s) (s_end - s) ds, and
            // swapping the order of integration turns that into a (s_end - s)-weighted average. Three
            // samples along the path with the weights that reproduce the triangle exactly for a
            // quadratic field profile; two fixed-point steps, since the path itself depends on the
            // field. B_bend is normalised to Bz(0,0), so the scale factor is the origin field bf.
            {
            ...
            if (p0.valid) {
              ...
              // Everything above sizes the road; this moves it. The walk propagates the curvature the
              // fit published at the production vertex while the real track's curvature grows along the
              // path, so at the layer it sits inside that circle by the double integral of the growth.
              // In the band's local frame (u = radial offset, outward positive) the offset is
              //   uE = shEU + shEUp*ds - 0.5*(shEDk*ds^2 + shEK*S2_arr)
              // with S2_arr the second moment about the arrival end on a transverse lever. Projected on
              // the two rows with the crossing geometry; a bias, never a variance.
              ...
              // --- the hole hypothesis ---------------------------------------------------------------
              // "This layer produced no usable hit" competes in the same currency as a candidate. The
              // PDA no-detection weight is chi2_hole = 2 ln[ eta_L / ((1 - eta_L eps) nu) ] with
              // nu = rho_d (2 pi)^{d/2} |S|^{1/2}; the |S| half is per-candidate and is added at the
              // commit site, together with the local/layer density ratio the scan measures. eta_L and
              // rho are measured detector properties (no |eta| axis: a layer's row is dominated by
              // whichever |eta| population supplies most of its samples, which is why the forward rows
              // are the sub-2.4 population's -- see the report).
          ...
          for (int round = 0; round < 2; ++round) {
            ...
            for (auto element : cms::alpakatools::uniform_group_elements(acc, j, nC * nLanes)) {
              ...
              if (pred.valid) {
                ...
                // The occupancy the scan itself sees. The hole is priced with the number of random
                // clusters expected where this candidate is, and a layer average understates that
                // inside a jet by as much as the jet is dense. The patch is the scan's own: the phi
                // bins it traverses, and the SAME angular span in pseudorapidity about the crossing --
                // an (eta, phi) cell, which is the shape detector occupancy is uniform in. What the
                // count is compared against is this layer's OWN occupancy in this event, spread
                // uniformly over the same patch (shOccNorm below), so the result is a dimensionless
                // contrast: the measured density tables keep setting the scale, the scan says only how
                // much denser this neighbourhood is than the layer it sits on. Every lane derives the
                // patch from the window it already computed; the counting is one compare per hit the
                // scan reads anyway, and the sum comes out of the argmin's own tree reduce.
            ...
            for (auto element : cms::alpakatools::uniform_group_elements(acc, j, nC * nLanes)) {
              ...
              if (shDerLayOn && gHit >= 0 && (shDerHoleK > -1e29f || shDerHoleKRaw > -1e29f || shDerHoleK3 > -1e29f)) {
                ...
                // Price it with the occupancy the scan MEASURED here rather than with the layer's
                // average alone: nu is the number of random clusters expected where this candidate
                // sits, and inside a jet the average understates it by the jet's own contrast.
                // laneOcc[0] is the patch count summed over the lanes and shOccNorm the count the same
                // patch would hold if this layer's clusters were spread over it uniformly, so their
                // product is that contrast and the constant shifts by -2 ln of it. Only the areal part
                // of the 3-dof density moves -- the bend dimension is a property of the stub, not of
                // the neighbourhood. With no cluster counted there is no winner either, so the floor
                // at one only guards a patch clipped by the layer's envelope.
              ...
              if (gHit >= 0) {
                ...
                sh.recomputeHelix(acc, shSegBf);
                // The energy-loss centre re-anchors by its own recursion, on the same gap moments the
                // process noise just used, so the two states never disagree about how much material
                // was traversed:
                //   u      <- u + u'*ds - (dkappa*ds^2 + K*S2)/2    [an identity, not a truncation]
                //   u'     <- u' - (dkappa*ds + K*S1)
                //   dkappa <- dkappa + K*W
                // It is not reset by the measurement update: the residual fed to the filter is the
                // corrected one, so the running helix tracks the constant-kappa_0 reference trajectory
                // the fit published while these three carry the true track's growing offset from it.
                if (shEK > 0.f) {
                ...
                // statistic and threshold) and the best passing hit is accepted as a second extra
                // on this layer. Lane-0-serial over a few hits per module, round 1 only.
                // The partner is a second raw-OT extra of one cluster, so it needs one more cluster of
                // room under the matching bound.
  ...
  void launchMergerAttach(Queue& queue,
    ...
    // the range the count/fill pre-gate iterates, so the track capacity is a host-known upper bound
    // and no candidate-count readback is needed. The cost of the looser bound is memory and grid
    // width, not correctness: candCapacity also sizes the stage-B RefitScaffold arrays and the
    // per-candidate launch grids.
    auto extBufs =
