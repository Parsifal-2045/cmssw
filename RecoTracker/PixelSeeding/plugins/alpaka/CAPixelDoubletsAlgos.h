      ...
      */
      ...
      for (auto kk = kl; kk != khh; incr(kk)) {
        ...
        for (uint32_t pIndex : cms::alpakatools::independent_group_elements_x(acc, maxpIndex)) {
          ...
          // Stub-stub compatibility. A stub measures the track direction at its own radius,
          // psi = phi + beta with tan(beta) = r * dPhiDr. For any circle through the two hits the
          // two tangents mirror the chord, psi_inner + psi_outer = 2 * phi_chord, whatever the
          // curvature and whatever the impact parameter; that sum is what is tested. Comparing the two
          // stub curvatures instead would compare kappa/2 + d0/ri^2 with kappa/2 + d0/ro^2, a residual
          // that grows with d0 and rejects real displaced tracks. The residual is written as
          // sin(gamma_i + gamma_o) with the unnormalised tangent T = (x - r*d*y, r*d*x + y), so it
          // costs no trigonometry; sigma from d(beta)/d(dPhiDr) = r/den on the precision-only bend
          // error.
          // Pairs involving pixel hits or PHitOnly stubs are skipped; a negative per-pair
          // stubSigmaCut disables the cut.
          if constexpr (std::is_same_v<pixelTopology::Phase2OTStubs, TrackerTraits>) {
            ...
            if (stubSigmaCut > 0.f && ll.isOT()[inner] && isStub(hh, oi) > 0.f && isStub(hh, i)) {
              ...
              // The tangent-sum residual above is free of the impact parameter by construction, which
              // also means it does not ask the two stubs to agree on the curvature at all: any circle
              // through the two hits passes, however far from the beam line it is. The iteration does
              // bound that distance, and the window below applies that bound. A stub measures
              // kappa/2 + d0/r^2, so for |d0| <= maxStubTip the two stub curvatures may differ by at
              // most maxStubTip |1/ri^2 - 1/ro^2| plus the measurement error. The acceptance enters as a
              // bound on the residual, not as a variance: it is a range, not a Gaussian, so multiplying
              // it by the sigma count would widen the window n times its own acceptance.
          ...
          // Pixel-to-stub direction consistency: with only the outer hit a stub, compare the kappa of
          // the pixel-stub geometry with the stub's own kappa measurement. Two points and one
          // direction fix a circle completely, so unlike the stub-stub case above no residual exists
          // that is free of the impact parameter: the chord measures kappa/2 + d0/(ri ro) and the
          // stub kappa/2 + d0/ro^2. The difference is d0 (ro - ri)/(ri ro^2), and maxStubTip is the
          // transverse displacement the iteration admits, added to the error so that the test stays a
          // sigma count and stops rejecting the displaced tracks its own arm is looking for.
          // Reuses the per-pair stubSigmaCut as the threshold, so a negative maxStubCurvSigma disables it.
          if constexpr (std::is_same_v<pixelTopology::Phase2OTStubs, TrackerTraits>) {
            auto stubSigmaCut = doubletCuts.maxStubCurvSigma()[pairLayerId];
