...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  // partial is identically zero by rotation invariance and is not returned; `g` is the value
  // extBendPredDPhiDr returned, passed in to carry the branch sign. Returns false if degenerate.
  //
  // rho = 1/(invPt*bf), T = d0 + rho, A = |T| = |centre|, P = |rho|, rho_i = drho/d(invPt) =
  // -rho^2 bf, dA/dd0 = sgn(T), dA/d(invPt) = sgn(T) rho_i.
  // Barrel (radius R), with Aa = A^2 - rho^2, c = (R^2 + Aa)/(2 A R), s = sqrt(1 - c^2),
  // c' = 1/(2A) - Aa/(2 A R^2) and g = -sigma c'/s, differentiating ln|g| = ln|c'| - ln s:
  //     dg/du = g ( (dc'/du)/c' + c (dc/du)/s^2 ),                                    (1)
  //     dc/du = (dAa/du)/(2 A R) - c (dA/du)/A,   dc'/du = -(dA/du / A) c' - (dAa/du)/(2 A R^2),
  //     dAa = 2 A dA - 2 rho drho.  A, Aa and R do not depend on cot or z0, hence
  //     dH_b/dcot == dH_b/dz0 == 0 exactly.                                            (2)
  // Endcap (surface z), with arc = (z - z0)/cot, beta = arc/rho, N = P - A cos(beta),
  // D = -A sin(beta), r^2 = A^2 + P^2 - 2 A P cos(beta) and
  //     g = dphi/dr = N / (D r),                                                        (3)
  ...
  //     dP/d(invPt) = sgn(rho) rho_i, dbeta/d(invPt) = arc bf, dbeta/dcot = -beta/cot,
  //     dbeta/dz0 = -1/(rho cot).
  template <typename TAcc>
  ...
  // The five measurement rows of one layer crossing, all from the same closed-form crossing code in
  // VecDual arithmetic: no Jacobian and no sign is hand-derived. Written for the surface the walk
  // predicts on (barrel: fixed R; endcap: fixed z) at the current state, and used for the whole visit
  // (the walk is linearised at the reference crossing anyway).
  //   Hphi[5] = d(r * phi)/d{phi0, tip, invPt, cot, zip}   [cm per unit parameter]
  //   Hsec[5] = d(sec)/d{...}, sec = z in the barrel and r in the endcap
  // For the reader: in the barrel the phi row reduces to r*(1, -1/r, -bf*s^2/(2r), 0, 0) and the
  // secondary row to (cot*ds/dphi0, ..., s, 1); the circle terms of the secondary row and the
  // (cot, zip) terms of the endcap phi row are exactly what the block-diagonal shortcut dropped.
  ...
  struct RunningHelix {
    ...
    // Rank-structured multiple-scattering process noise of one traversed gap, in the PERIGEE frame.
    // A kink of variance q at perigee arc s_k enters the state as (dphi0, dtip) = (angle, +angle*s_k):
    // with this walk's own rows, d(r*phi) = r*(dphi0 - dtip/r) = r*angle*(1 - s_k/r), the textbook
    // downstream shift of a kink at s_k. The (cot, zip) pair takes (dcot, dzip) = (a, -a*s_k), since
    // z = zip + s*cot must be unchanged at s = s_k. Summed over the gap's material this gives
    //   Q(phi0,tip) = cPhi * [[W, S1],[S1, S2]],   Q(cot,zip) = cCot * [[W, -S1],[-S1, S2]],
    // with W, S1, S2 the zeroth/first/second moments of rho*dl in the perigee arc s_k (NOT about the
    // arrival end: the caller converts). Curvature is untouched -- scattering is elastic.
