...
namespace extDerivedTables {
  ...
  // The duplicate removal does NOT gate at eps. Its test asks the opposite question of the walk's:
  // the walk accepts a hit that falls inside the probability mass eps of a correct hit, while the
  // duplicate test keeps two tracks apart only when their fits DISAGREE -- so its threshold is a
  // rejection significance. It is taken at one-sided 5 sigma (p = 5.733e-7), which at 1 dof is
  // exactly 25: the same 5 sigma a per-parameter test would ask for, taken jointly so the parameter
  // correlations are used instead of ignored. Two tracks of the same particle share most of their
  // hits, so the sum of their covariances overstates the spread of their difference; a tighter
  // threshold splits real duplicates, and a surviving duplicate is the more expensive error.
