// The wrong-stub test of the General Broken Lines refit, against tracks propagated through the material
// map and read out with the outer tracker's own sensors and stubs.
//
// The refit's outlier stage drops the measured node whose smoothed-residual pull is past the 99.9 %
// chi2(2) bound. A stub carries a second, independent measurement the stage did not use: the bend
// dPhiDr = (phi_outer - phi_inner) / dr_effective between the two sensors of its stack, which is the
// track's stereo angle at that radius and therefore its curvature. This test compares that bend with the
// one the FITTED circle has at the stub's radius -- the producer's own bend model, with the impact
// parameter carried in the circle centre -- and lets a stub past the 99.9 % chi2(1) bound be dropped by
// the same mechanism as a position outlier.
//
// Measured here:
//   * the model itself: the pull (measured - predicted)/sigma over clean stubs, per module class, which
//     must be centred at 0 with unit width if the bend model and its sign conventions are right;
//   * the false-drop rate at the bound on clean tracks (must be ~0.1 %), also with the declared error
//     deliberately 2x too loose and 2x too sharp, which is the uncertainty the PS bend error carries;
//   * the drop efficiency on a deliberately wrong stub (the bend of a 2 GeV track placed on a 1 GeV one);
//   * that a right stub with a 3-sigma bend fluctuation is kept (3 sigma is chi2 9 < 10.83);
//   * that the refit's d0, z0, phi, cot and |kappa| pulls do not move on tracks with no wrong stub;
//   * device vs host twin of the prediction.
//
// Generation (testBLStubCovDevice's generator, as testBLRelinDevice uses it): RK4 from the origin in a
// constant solenoid field with Highland kinks per crossed material lump; macro-pixel (PS) and strip (2S)
// readout uniform over the cell, with the production stub error factors. The bend of a crossing is the
// producer's own model evaluated on the TRUE local direction, smeared with the class bend precision.
