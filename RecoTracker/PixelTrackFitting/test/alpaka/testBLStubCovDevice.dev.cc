// The covariance the fast BrokenLine fit declares for an outer-tracker-only track, against tracks
// propagated through the material map and read out with the outer tracker's own sensors.
//
// The displaced iteration fits stub tracks whose first hit sits at r ~ 23 cm and whose state is
// extrapolated back over an empty inner lever. Two things set the error it declares at the closest
// approach: the measurement variances of the stubs (macro-pixel and strip, the strip's along-strip
// coordinate uniform over its length) and the scattering kinks, upstream of the first hit and between
// the layers. This oracle generates a track in a constant field -- so that the circle-plus-line model
// IS the trajectory and nothing but the covariance is under test -- scatters it in the map, reads it
// out with uniform sensor errors, and fits it with the production preparer and the two fits.
//
// Three variants are fitted from the same events:
...
// There is no measurement-only variant: a broken line whose material is removed has no kink penalty, so
// every node's offset becomes free and the fit is not the straight line such a test would need.
//
// The kinks are generated as the PDG formula prescribes: Highland's logarithm at the whole thickness the
// particle crosses, not at each lump, which is what makes a chain of thin scatterers add up to the same
// angle as one thick one.
//
// Reported per (layout, pt, eta): the spread of each fitted parameter over the replicas divided by the
// error the fit declares for it -- the pull sigma, formed without the perigee sign conventions since the
// spread does not need the true value.
