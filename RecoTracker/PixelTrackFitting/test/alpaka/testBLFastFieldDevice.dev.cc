// The fast BrokenLine fit's magnetic-field treatment (interface/alpaka/BrokenLine.h, fitCorrections on),
// against tracks propagated by RK4 in the real CMS solenoid lattice (test/blBFieldMapFixture.h, the same
// values the EventSetup product hands the kernels).
//
// The fit models one circle in a single constant bending field plus a straight line in (s,z). The real
// field breaks both: B_r turns the dip angle along the path (dlambda/ds = -(q/p) B_r sin(alpha), odd in z),
// and Bz(r,z) falls by ~7.5 % towards the endcaps, so the curvature is not constant along the track. The
// production package removes both as deterministic offsets of the fit's coordinates. Here a track is
// propagated in the lattice, sampled on the tracker's layer radii and disc planes, and fitted:
//
//   1. noiseless, no scattering: with the field rows on, the fitted z0 and cot(theta) must reproduce the
//      generated ones to far better than the detector resolution; with them off the fit carries the z-odd,
//      charge-even bias of A/R1 (~0.1-0.2 mm in z0, ~0.1 mrad in theta, per GeV/pt, at |eta| 1.0-1.7);
//   2. the fitted pt is the same whether the curvature is converted with the origin field or with the
//      track's hit-averaged effective field, i.e. the profile row -- not the choice of the single scalar --
//      is what carries the field's shape;
//   3. with hit noise and simulated multiple scattering the pulls of z0 and cot(theta) stay near one and
//      their means carry no z-odd component.
//
// The same assertions run on every backend, so the serial (host) and the GPU builds are checked against one
// oracle; the noiseless fitted values are additionally required to agree between the two variants' shared
// arithmetic to the printed precision.
...
namespace {
  ...
  constexpr int kNOt = 6;   // hits of the outer-tracker-only (displaced) layout
  ...
  //!< layer it crosses, true = the outer-tracker-only layout the displaced iteration fits (first hit at
