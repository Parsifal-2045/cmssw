...
namespace {
  ...
  void runFixture(Queue& queue,
                  ...
                  double tolTwin,
                  double tolDesy,
                  const blMaterialMap::Map* rhoDev) {
    using O = Out<N>;
...
TEST_CASE("GBL device chain vs the DESY oracle for the " EDM_STRINGIZE(ALPAKA_ACCELERATOR_NAMESPACE) " backend",
  ...
  // The Geant4 material map: no EventSetup needed, the T35 map is read from its binary file and
  // uploaded here exactly as the ES product would.
