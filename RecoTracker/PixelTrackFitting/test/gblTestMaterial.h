...
// backend.
// The table itself is not duplicated: theMap() reads the shipped T35 file.
...
namespace gblTestMaterial {
  ...
  // The shipped T35 file, read once into the map the accessors read.
  inline const blMaterialMap::Map& theMap() {
    static const blMaterialMap::Map map = [] {
      blMaterialMap::Map m;
  ...
  inline void segmentWalk(double r0,
    ...
    const blMaterialMap::Map& rho = theMap();
    ...
    for (int cell = 0; cell < kMaxCells && t < 1.; ++cell) {
      ...
      if (tn > t) {
        ...
        if (q > 0.f) {
          ...
          if (col != nullptr) {
            const blMaterialMap::DeDx d = blMaterialMap::dedxAt(rho, rm, zm);
