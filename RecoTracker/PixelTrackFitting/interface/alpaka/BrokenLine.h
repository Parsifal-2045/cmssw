...
// one package:
//   - material per gap from the Geant4 map rho(r,z) (BLMaterialMap.h) by an exact walk of the map's cells,
//     split between the gap's two end nodes so that its total and first moment are reproduced
//     (segmentXX0Endpoint); a node with no material carries no kink; the innermost term walks the map from
//     the track's own closest approach to the beam line up to hit 0 along its 3-D path and enters as a pure
//     angle at hit 0, together with the share of gap 0 the partition assigns to node 0 (which has no kink
//     degree of freedom of its own);
...
//   - the deterministic ionization-loss offset of the circle residuals, charged per node from the map's own
//     dE/dx columns (see circleFit);
...
//     is removed from the circle residuals (circleFit). Both are the rows the GBL refit carries
//     (GeneralBrokenLine.h), in the same sign convention;
...
namespace ALPAKA_ACCELERATOR_NAMESPACE::brokenline {
  ...
  */
  ...
  // Exact cell walk of the material map along the straight (r,z) chord (r0,z0)->(r1,z1), r,z in cm. The
  // chord is cut at every radial (kDR) and z (kDZ) cell boundary it crosses and every piece is charged with
  // its own cell's density times its exact length, so no cell is skipped or counted twice; a fixed-step
  // march at ~0.5 cm instead aliases the 0.5 cm radial lattice (13 % rms per gap).
  // Outputs the chord length L and the three moments about the ARRIVAL end (r1,z1), with d the distance
  // from a point of the chord to that end:  W = int rho dl, S1 = int rho d dl, S2 = int rho d^2 dl.
  // Per cell the moments are exact in closed form: with the cell spanning the chord parameter [ta,tc] and
  // a = 1-ta, c = 1-tc, the integrals of rho, rho*d and rho*d^2 are rho L (a-c), rho L^2 (a^2-c^2)/2 and
  // rho L^3 (a^3-c^3)/3.
  // `path3D` > 0 rescales the result to the track's real 3-D path over the segment: the map holds X/X0 per
  // cm of 3-D path and the chord is shorter than the path of a curved track. Under d -> (path3D/L) d the
  // three moments scale by k, k^2 and k^3 with k = path3D/L, which leaves the shape quantities (fDep, w1)
  // unchanged and stretches the lever d1 into path units.
  ...
  */
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE void __attribute__((always_inline)) prepareBrokenLineData(const TAcc& acc,
    ...
    // Radial-field (B_r) row of the equation of motion, the same row the GBL refit carries
    // (GeneralBrokenLine.h): the real field turns the dip angle along the path,
    //     dlambda/ds = -(q/p) B_r sin(alpha),
    // with alpha the angle from the position azimuth to the momentum azimuth. B_r is odd in z, so a
    // Bz-only line model buys a z-odd dip-angle and z0 bias (~0.1 mrad / 0.1-0.2 mm per GeV/pt at
    // |eta| 1.0-1.7). The resulting z displacement is deterministic, so it is subtracted from the measured
    // z coordinates here: the line fit then sees a straight trajectory and its parameters are the state at
    // the reference PCA, where the accumulation starts (as in the refit, which removes its offsets at the
    // PCA node). Value only -- no fit parameter, no change to the weights or the covariance.
  ...
  */
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE void circleFit(const TAcc& acc,
    ...
    // place.
    // The bending field's profile rides in the same offset. The fit models one circle of curvature
    // kappa = q*bField/pT with bField the track's single effective field, while the real curvature follows
    // the map, kappa(s) = kappa * B_bend(r,z)/bField (B_bend as in BLBFieldMap.h, the same law the effective
    // field averages). Only the profile's SHAPE matters: a constant offset is absorbed exactly by the fitted
    // Delta-kappa, which is why the effective field alone leaves the shape unmodelled. This is the GBL
    // refit's azimuth row, expressed as the curvature increment of a circle fit.
