...
namespace ALPAKA_ACCELERATOR_NAMESPACE::brokenline {
  ...
  */
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE double segmentXX0GapSplit(
      const TAcc& acc, const blMaterialMap::Map* map, double r0, double z0, double r1, double z1, double& d1, double& w1) {
    ...
    for (int k = 0; k < nseg; ++k) {
      ...
      const double q = blMaterialMap::rhoAt(*map, float(r0 + f * (r1 - r0)), float(z0 + f * (z1 - z0))) * w;
      W += q;
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE void __attribute__((always_inline)) prepareGblFitData(const TAcc& acc,
    ...
    for (u_int g = 0; g < n - 1; g++) {
      ...
      const double xx0 = segmentXX0Moments(
          acc, map, rOf(g), hits(2, g), rOf(g + 1), hits(2, g + 1), d1, w1, 0., matCol ? matCol + g : nullptr);
    ...
    results.innerXX0 = useInner ? segmentXX0(acc, map, 0., 0., rOf(0), hits(2, 0), 0., &results.innerCol) : 0.;
  }
