...
namespace ALPAKA_ACCELERATOR_NAMESPACE::generalBrokenLine {
  ...
  // and of ln rho_e -- exactly what ElossColumn carries. Evaluating the density effect at the column's
  // effective (ln I, ln rho_e) is exact in the asymptotic region and second order in the spread of Cbar over
  // the materials of the path (< 1 % for the tracker's mix). delta follows Sternheimer's parametrization with
  // the generic Sternheimer-Peierls parameters Geant4 uses for materials without tabulated ones
  // (PDG RPP 34.2.5; Sternheimer & Peierls, Phys. Rev. B 3 (1971) 3681; Bichsel, Rev. Mod. Phys. 60 (1988) 663).
  template <typename TAcc>
  ...
  */
  ...
  // Stub bend the fitted circle predicts at a stub node, in the producer's own units.
  //
  // The outer-tracker stub producer (OTStubFormationVectorHitStyleKernels.h) publishes
  // dPhiDr = (phi_outer - phi_inner) / dr_effective with dr_effective = separation / (nR + nZ z/r), where
  // (nR, nZ) is the module normal in the (r, z) plane (flat barrel (1,0), disc (0,+-1), tilted barrel
  // pointing). Following the fitted circle -- centre (cx, cy), radius R, charge q, cot(theta) -- across
  // that same separation gives the azimuth change sin(beta) * ds / r with the transverse path
  // ds = separation / (nR cos(beta) + nZ cot(theta)), hence
  //
  //     dPhiDr_fit = sin(beta) (nR + nZ z/r) / (r (nR cos(beta) + nZ cot(theta))),
  //
  // with beta the angle of the track's transverse direction to the radial one at the hit (cos(beta) along
  // e_r, sin(beta) along e_phi). The separation cancels, so no module-level geometry is needed. The impact
  // parameter sits in the circle centre, so a displaced track is modelled and not approximated by kappa/2.
  // Returns false when the node is degenerate (on the axis, or the track running along the module plane).
  template <typename TAcc>
