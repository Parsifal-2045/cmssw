...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  // goes left.
  //
  // Two loop orders, selected at compile time from the accelerator:
  //  * single-thread-per-block (CPU) backends: tree-outer / track-inner over a tile of up to
  //    kForestCpuTrackTile tracks, so a tree's contiguous nodes are cached once per tile instead of
  //    being re-fetched per track.
  //  * other backends: track-per-thread, the forest walked tree by tree inside the thread.
  // Both accumulate baseLogit + leaf(tree 0) + leaf(tree 1) + ... in ascending tree order, so they
  // produce the same fp32 margin bit for bit. Rows in [nValid, maxPreselectedTracks) are untouched.
  ...
  // ------------------------------------------------------------------------------
  // Warp-per-track variant of TreeScoreKernel for the GPU backends: the forest sum is over
  ...
  //
  // Determinism: the lane -> tree assignment is fixed and the reduction is a fixed ascending
  // butterfly (shfl from source lane laneId ^ off, off = 1,2,4,...), so the score is stable run to
  // run at the bit level for a given backend. The tree sum associates differently from
  // TreeScoreKernel (balanced tree vs left fold), so the two agree only to fp32 rounding; with
  // warpSize == 1 lane 0 handles every tree in TreeScoreKernel's order.
  //
  // Work division: Acc2D, Y indexes tracks (uniform_elements_y), X the warpSize lanes
  // (getIdx<Block,Threads>[1]); all X lanes of a warp share the same Y, so the warp is never split
  // across tracks and every shfl sees a full, converged warp.
