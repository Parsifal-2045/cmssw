 ...
 *
 * Accelerator module performing displaced HighPurity pixel-track selection with a
 ...
 *
 * The model is an XGBoost forest exported to a compact binary (int8 split-feature, fp32
 * threshold/leaf value, int32 children, per-tree root offsets, base logit). It is loaded once per
 * process into a shared GlobalCache (DispTreeCache), lifted to the device as a read-only buffer and
 * traversed by a hand-written alpaka kernel, so the O(10 MB) model lives once per device rather than
 * once per stream.
 */
