 ...
 *  including kinematic variables, track quality metrics, and optionally Standalone Muon
...
std::vector<float> MuonOITracksDNNSelector::extractFeatures(const reco::Track& track,
  ...
  // 25: Low pT indicator (sigmoid around 5 GeV)
...
void MuonOITracksDNNSelector::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  const std::string metname = "RecoMuon|L3TrackFinder|MuonOITracksDNNSelector";
