...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  // The Phase-2 stubs topology takes the pixel hit errors from the per-module genError parameters rather than from
  // the cluster-size table, so it needs its own set of parameters (see errorFromDB in pixelCPEforDevice.h).
