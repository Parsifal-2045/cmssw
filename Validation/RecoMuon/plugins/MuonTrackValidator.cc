...
void MuonTrackValidator::bookHistograms(DQMEDAnalyzer::DQMStore::IBooker& ibooker,
  ...
  for (unsigned int ww = 0; ww < associators.size(); ww++) {
    for (unsigned int www = 0; www < label.size(); www++) {
      ...
      h_assocdR.push_back(
          ibooker.book1D("num_assoSimToReco_dR", "N of associated tracks (simToReco) vs dR", bindR, mindR, maxdR));
      h_assoc2dR.push_back(
          ibooker.book1D("num_assoRecoToSim_dR", "N of associated (recoToSim) tracks vs dR", bindR, mindR, maxdR));
  ...
  for (unsigned int ww = 0; ww < associators.size(); ww++) {
    for (unsigned int www = 0; www < label.size(); www++) {
      ...
      std::optional<double> dR = [&]() -> std::optional<double> {
        if (trackCollectionSize == 2) {
          return reco::deltaR(trackCollection->at(0), trackCollection->at(1));
        }
        return std::nullopt;
      }();
