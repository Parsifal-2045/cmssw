...
void MuonTrackValidator::bookHistograms(DQMEDAnalyzer::DQMStore::IBooker& ibooker,
  ...
  std::cout << "Associator size: " << associators.size() << "\n";
  std::cout << "HistoParameters size: " << histoParameters.size() << "\n";
  ...
  for (unsigned int ww = 0; ww < outerLoopSize; ++ww) {
    for (unsigned int www = 0; www < label.size(); ++www, ++w) { // increment w here to account for continue
      ...
      for (edm::View<Track>::size_type i = 0; i < trackCollectionSize; ++i) {
        ...
        std::cout << "FILLING RECO HISTOGRAMS FOR TRACK " << i << std::endl;
        ...
        std::cout << "ABOUT TO DO MUO/TRK PLOTS" << std::endl;
        if (histoParameters[www].do_MUOhitsPlots) {
          std::cout << "MUO PLOTS" << std::endl;
          std::cout << "w: " << w << ", www: " << www << ", i: " << i << ", muonPlotIndex: " << muonPlotIndex
                    << std::endl;
          std::cout << "HistoV size: " << nDThits_vs_eta.size() << std::endl;
          std::cout << "HistoV size: " << nCSChits_vs_eta.size() << std::endl;
          std::cout << "HistoV size: " << nRPChits_vs_eta.size() << std::endl;
          ...
          if (useME0_) {
            std::cout << "ME0s" << std::endl;
            nME0hits_vs_eta[muonPlotIndex]->Fill(xetaRec, track->hitPattern().numberOfValidMuonME0Hits());
          }
          std::cout << "ALL MUO DONE" << std::endl;
        }

        if (histoParameters[www].do_TRKhitsPlots) {
          std::cout << "TRK PLOTS" << std::endl;
          std::cout << "w: " << w << ", www: " << www << ", i: " << i << ", trackerPlotIndex: " << trackerPlotIndex
                    << std::endl;
